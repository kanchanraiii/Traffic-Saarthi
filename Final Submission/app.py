...
with st.sidebar:
    selected = option_menu("Traffic Saarthi",
                           ["Home", "Route Optimization", "YOLO Real Time Congestion"],icons=["house-door","pin-map","signpost-split"],
                           default_index=0)

if selected == "Home":
    st.title("Welcome to Traffic Saarthi")
    st.write("**Smart Routes for Smart Driver!**")
    ...
    if uploaded_file is not None:
        if uploaded_file.type in ["image/jpg", "image/jpeg", "image/png"]:
            ...
            if st.button("Predict Congestion", key="predict_video"):
                ...
                st.write(f"{congestion_level} ({avg_detections_per_frame:.2f} vehicles per frame on average).")
